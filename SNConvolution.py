...
def max_singular_value(W, u=None, Ip=1):
    ...
    sigma = tf.sum(tf.matmul(tf.matmul(_v,W),tf.transpose(_u)))
    return sigma, _u, _v
...
class SNConvolution2D():
    ...
    def __init__(x, cnum, ksize=5, stride=2,dilation=1, name='conv', training=True, padding='SAME', Ip=1):
        ...
        self.name = name
        self.w = tf.get_variable("kernel", shape=[ksize, ksize, x.get_shape()[-1], cnum], initializer=tf.random_normal_initializer(mean=0.0, stddev=0.02),regularizer=None)
        self.b = tf.get_variable("bias", [cnum], initializer=tf.constant_initializer(0.0))
        self.u = tf.get_variable("u", [1, cnum], initializer=tf.random_normal_initializer(), trainable=False)
        self.stride = stride
    ...
    def conv(self):
        ...
        if self.padding == 'SYMMETRIC' or self.padding == 'REFELECT':
            ...
            padding = 'VALID'
        x = tf.nn.conv2d(
        input=self.input, filter=self.W_bar, strides=self.stride, dilations=self.dilation, #Double the cnum, because of split later
